<?php
	...
	if(isset($_GET['loans'])){
		...
		while($row=mysqli_fetch_assoc($sql)){
			...
			$pic=strlen($names[$row['client']]['photo'])>1?'data:image/jpg;base64,'.$names[$row['client']]['photo']:'assets/img/user.png';
			$day=date("d-m-Y",$row['time']); $paid=number_format($row['paid']);
			
			$trs.="<tr valign='top'><td style='width:50px'><img src='$pic' style='width:40px;border-radius:50%'></td><td>$name<br>
